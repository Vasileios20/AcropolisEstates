...
const ListingFilesDrawer = ({ visible, onClose, listingId, isShortTerm = false }) => {
    ...
    const fetchFiles = async () => {
        try {
            ...
            antMessage.error(t('admin.fileSelection.errorFetchingFiles'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpload = async () => {
        if (!fileToUpload) {
            antMessage.warning(t('admin.fileSelection.pleaseSelectFile'));
            return;
        }
    ...
    return (
        <Drawer
            title={
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Title level={4} style={{ margin: 0, paddingTop: '54px' }}>
                        {t('admin.fileSelection.listingFilesAndDocuments')} - #{listingId}
                    </Title>
        ...
        >
            {/* Upload Section */}
            <Card title={t('admin.fileSelection.uploadNewFile')} style={{ marginBottom: '24px' }}>
...
export default ListingFilesDrawer;